...
<Steps>
  ...
  <Step title="Generate an API key">
    Open the [Developer Dashboard](https://knouds.ai/app/api/keys) (or **Settings → API Keys**, which redirects). Click **Create Key** and pick a preset:

    - **Workflow Deploy** — `workflow:run` + `workflow:read`. The right preset for most production integrations.
    - **Read-only** — `workflow:read` only. Useful for audit/monitoring keys.
    - **Webhook receiver** — `webhook:receive`. Use when configuring outbound webhook delivery for `?async=true` runs.
    - **Full deploy** — `workflow:run` + `workflow:read` + `workflow:write`. Adds the ability to create/update/delete workflows via API.
