...
Long-running model and workflow invocations can opt into **async mode** by appending `?async=true` to the run endpoint. The submit returns in <500ms with an `executionId`; you then either poll `GET /api/executions/:id` or receive a webhook callback when the run completes.
