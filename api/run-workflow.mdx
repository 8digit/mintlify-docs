...
For long-running pipelines you can append `?async=true` to submit the workflow without blocking. The response returns in under 500ms with an `executionId`; poll [`GET /api/executions/:id`](/api/async-executions) or receive a webhook callback when the run completes.
