...
Append `?async=true` to submit the model run without blocking. Response returns in <500ms with an `executionId`. Supported on HTTP+polling defs (Seedance, Suno, gpt-image-2). Fal-backed and Anthropic-backed defs return `400 ASYNC_NOT_SUPPORTED`.
